...
export class CubismModelSettingJson extends ICubismModelSetting {
   ...
   */
  public getDisplayInfoFileName(): string {
    if (!this.isExistDisplayInfoFile()) {
      return '';
    }
   ...
   */
  protected isExistDisplayInfoFile(): boolean {
    const node: Value = this._jsonValue.at(FrequestNode.FrequestNode_DisplayInfo);
    return !node.isNull() && !node.isError();
  }
